...
export interface PlannerCandidate { id: string; kind: "action" | "route" | "locator" | "input" | "expectation" | "endpoint"; case_id: string; requirement_id?: string; feature_id?: string; scenario: string; route_id?: string; locator_id?: string; input_id?: string; endpoint_id?: string; action?: string; method?: string; path?: string; body?: unknown; step?: Record<string, unknown>; locator_ref?: Record<string, unknown>; origin?: string; strength?: "weak" | "normal" | "strong"; source?: "fixture" | "discovery" | "rule" | "manual"; confidence?: number; risk?: "read_only" | "mutating" | "destructive"; }
...
export interface PlannerSkeleton { case_id: string; requirement_id?: string; feature_id?: string; intent?: string; scenario: string; priority?: "P0" | "P1" | "P2"; route_id?: string; action_ids?: string[]; expectation_ids?: string[]; status?: string; }
...
export function validatePlannerOutput(input: PlannerInput, output: PlannerOutput, { allowedOrigin, production }: { allowedOrigin?: string; production?: boolean } = {}): ValidationResult {
  ...
  for (const selection of output.caseSelections) {
    ...
    const selectedStrengths = (selection.expectationIds || []).map((id) => input.candidates.expectations[id]?.strength);
    const semanticRequired = !["route_loads", "not_found_semantics", "cors_allows_operation"].includes(skeleton.intent || "");
    if (skeleton.scenario === "normal" && semanticRequired && !selectedStrengths.includes("strong")) errors.push(selection.caseId + ": normal scenario lacks strong semantic result assertion");
    if (skeleton.scenario === "normal" && !semanticRequired && !selectedStrengths.some((strength) => strength === "normal" || strength === "strong")) errors.push(selection.caseId + ": normal scenario lacks protocol result assertion");
  }
