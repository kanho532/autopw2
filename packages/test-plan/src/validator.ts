...
const ACTION_KEYS: Record<string, string[]> = {
  ...
  wait_for: ["action", "locator", "state", "timeout_ms"], api_request: ["action", "method", "path", "headers", "body", "save_as", "acceptable_statuses"],
...
function validateActionShape(action: string, step: Record<string, unknown>, errors: string[], prefix: string, context: PlanValidationContext): void {
  ...
  if (action === "api_request") { required("method"); required("path"); if (!["GET", "POST", "PUT", "PATCH", "DELETE", "OPTIONS", "HEAD"].includes(String(step.method))) errors.push(prefix + ": invalid HTTP method"); validateRelativePath(String(step.path || ""), prefix, errors); if (step.headers !== undefined && (!isObject(step.headers) || Object.values(step.headers).some((value) => typeof value !== "string"))) errors.push(prefix + ": headers must be string values"); if (step.save_as !== undefined && !validVariableName(step.save_as)) errors.push(prefix + ": invalid response variable"); if (step.acceptable_statuses !== undefined && (!Array.isArray(step.acceptable_statuses) || step.acceptable_statuses.length === 0 || new Set(step.acceptable_statuses).size !== step.acceptable_statuses.length || step.acceptable_statuses.some((value) => !Number.isInteger(value) || value < 100 || value > 599))) errors.push(prefix + ": acceptable_statuses must contain unique HTTP status integers"); }
