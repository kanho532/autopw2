export interface GateAuditLike { audit_status: "COMPLETE" | "INCOMPLETE"; evidence_complete?: boolean; cleanup_complete?: boolean; requirement_reconciliation?: "COMPLETE" | "MISMATCH"; }
...
export function evaluateGate({ audit, auditStatus, coverage, gatePolicy, issues = [], executionResults = [] }: { audit?: GateAuditLike; auditStatus?: "COMPLETE" | "INCOMPLETE"; coverage?: GateCoverageLike; gatePolicy?: GatePolicy; issues?: Record<string, unknown>[]; executionResults?: Array<{ stability?: string }> }): GateOutcome {
  ...
  if (status === "INCOMPLETE" || audit?.requirement_reconciliation === "MISMATCH" || audit?.evidence_complete === false || audit?.cleanup_complete === false) return { gate: "incomplete", exit_code: 2, reason: "structural audit or coverage reconciliation incomplete" };
