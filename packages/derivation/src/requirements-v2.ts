...
type BaseDraft = Pick<RequirementDraft, "feature_id" | "source_refs" | "evidence_refs" | "preconditions" | "confidence"> & { resource_id: string; operation_id: string; fields: FieldNode[]; operation: OperationNode };
...
export function deriveGraphRequirements(input: EngineInput): TestRequirement[] {
  ...
  for (const resource of graph.nodes.resources) {
    ...
    for (const operation of operations) deriveOperationRequirements({ resource, operation, operations, fields, factByEvidence, add });
    for (const field of fields) deriveFieldRequirements({ resource, field, operations, factByEvidence, add });
  }
...
function deriveOperationRequirements(context: { resource: ResourceNode; operation: OperationNode; operations: OperationNode[]; fields: FieldNode[]; factByEvidence: Map<string, string>; add(draft: RequirementDraft): void }): void {
  const { resource, operation, operations, fields, factByEvidence, add } = context;
  ...
  const base = baseDraft(resource, operation, fields, factByEvidence);
...
function deriveFieldRequirements(context: { resource: ResourceNode; field: FieldNode; operations: OperationNode[]; factByEvidence: Map<string, string>; add(draft: RequirementDraft): void }): void {
  const { resource, field, operations, factByEvidence, add } = context;
  ...
  for (const constraint of field.constraints) {
    ...
    const legacyPayloadReady = rule === "required" && field.name === "title" || rule === "enum" && field.name === "priority";
    const payload: RequirementPayloadStrategy = { kind: "constraint", field_id: field.id, rule, ...(Object.hasOwn(constraint, "value") ? { value: constraint.value } : {}), ...(constraint.values ? { values: constraint.values } : {}), proven: legacyPayloadReady, ...(!legacyPayloadReady ? { reason: "PAYLOAD_SYNTHESIS_PENDING" } : {}) };
...
function baseDraft(resource: ResourceNode, operation: OperationNode, fields: FieldNode[], factByEvidence: Map<string, string>): BaseDraft {
...
function detailRequirement(base: BaseDraft, resource: ResourceNode, operation: OperationNode, operations: OperationNode[]): RequirementDraft {
  const create = operations.find((item) => item.method === "POST"); const cleanup = operations.find((item) => item.method === "DELETE"); const status = declaredStatus(operation, 200); const fixture = crudFixture(create, operation, cleanup, Boolean(create?.request_schema_refs.length && base.fields.length)); const refs = mergedRefs(base.evidence_refs, create?.evidence_refs, cleanup?.evidence_refs);
  return { ...withoutFields(base), evidence_refs: refs, requirement_id: `req_${resourceSlug(resource)}_detail`, intent: "route_detail", scenario: "normal", priority: "P0", oracle: status ? { kind: "http", assertion: "created entity can be read by identity", details: { status } } : null, oracle_specification: statusOracle(operation, "created entity can be read by identity", status), fixture_strategy: fixture, payload_strategy: fixturePayload(create, base.fields), risk: "read_only", confidence: confidenceOf([resource, operation, ...(create ? [create] : []), ...(cleanup ? [cleanup] : [])]) };
}
...
function fixturePayload(create: OperationNode | undefined, fields: FieldNode[]): RequirementPayloadStrategy { return { kind: "schema", schema_refs: create?.request_schema_refs || [], field_ids: fields.map((item) => item.id), proven: false, reason: "PAYLOAD_SYNTHESIS_PENDING" }; }
function crudFixture(create: OperationNode | undefined, read: OperationNode | undefined, cleanup: OperationNode | undefined, payloadKnown: boolean): RequirementFixtureStrategy { return { kind: "resource_crud", operation_ids: [create?.id, read?.id, cleanup?.id].filter((item): item is string => Boolean(item)), create_operation_id: create?.id, read_operation_id: read?.id, cleanup_operation_id: cleanup?.id, proven: Boolean(create && read && cleanup && payloadKnown), ...(!create || !read || !cleanup ? { reason: "MISSING_RESOURCE_FIXTURE_OPERATION" } : !payloadKnown ? { reason: "FIXTURE_PAYLOAD_UNPROVEN" } : {}) }; }
