...
const PAGINATION = () => ({ type: "object", properties: { page: { type: "integer", minimum: 1 }, page_size: { type: "integer", minimum: 1, maximum: 100 }, total_items: { type: "integer", minimum: 0 }, total_pages: { type: "integer", minimum: 0 }, has_more: { type: "boolean" }, next_page: { type: ["integer", "null"], minimum: 1 } }, required: ["page", "page_size", "total_items", "total_pages", "has_more", "next_page"], additionalProperties: false });
export function buildSchemas() {
  ...
  S["mcp-status-view"] = obj("mcp-status-view", "Read-only status query result", {
    ...
    progress_pct: { type: "number", minimum: 0, maximum: 100 }, counts: { type: "object", properties: { planned: { type: "integer", minimum: 0 }, started: { type: "integer", minimum: 0 }, terminal: { type: "integer", minimum: 0 } }, required: ["planned", "started", "terminal"], additionalProperties: false }, by_tier: { type: "object", additionalProperties: { type: "object", additionalProperties: true } }, by_batch: { type: "object", additionalProperties: { type: "object", additionalProperties: true } }, next_action: str("Recommended next action for the agent"), poll_after_ms: { type: "integer", minimum: 0, maximum: LIMITS.pollAfterMsMax.min }, stale: { type: "boolean" }, interrupted: { type: "boolean" }, recent_events: arr({ type: "object", additionalProperties: true })
  ...
  S["mcp-result-view"] = obj("mcp-result-view", "Final result after GATED", {
    kind: { type: "string", const: "ok" }, schema_version: VER(), run_id: ID("runId"), phase: { type: "string", const: "GATED" }, gate: enumRef("gate"), audit_status: enumRef("auditStatus"), results_ref: ART(), report_ref: ART(), gate_summary: { type: "object", additionalProperties: true }, issues: arr({ type: "object", additionalProperties: true }), pagination: PAGINATION()
  ...
  S["evidence-manifest"] = obj("evidence-manifest", "Evidence index + redaction state", { execution_id: ID("executionId"), items: arr({ type: "object", additionalProperties: true }), redacted: { type: "boolean" }, redaction_status: { type: "string", enum: ["COMPLETE", "INCOMPLETE"] } }, ["execution_id", "items", "redacted"]);
  ...
  S["results"] = obj("results", "Machine gate source of truth", { schema_version: VER(), run_id: ID("runId"), gate: enumRef("gate"), audit_status: enumRef("auditStatus"), exit_code: { type: "integer" }, results_ref: ART(), report_ref: ART(), summary: { type: "object", additionalProperties: true }, issues: arr({ type: "object", additionalProperties: true }) }, ["schema_version", "run_id", "gate", "audit_status", "exit_code", "results_ref"]);
