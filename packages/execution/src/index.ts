...
import Ajv from "ajv";
import { chromium, firefox, webkit, type APIResponse, type Browser, type BrowserContext, type BrowserType, type Locator, type Page } from "playwright";
...
export class PlaywrightPlanRunner {
    ...
    try {
      ...
      try {
        ...
        if (page && (item.kind === "ui" || item.kind === "hybrid")) {
          if (page.url() === "about:blank" && !testFailure) testFailure = classifyFailure(failureError("UI case completed without page navigation", "UI_PAGE_NOT_NAVIGATED", "navigated page", "about:blank"), "goto", "test", generatedPlan);
  ...
  private async executeStep({ page, context, step, baseUrl, scopes, network, storage, runId, caseId, consoleErrors, fixtureVariant, deadline }: ExecuteContext): Promise<{ summary?: unknown; endpoint_ref?: string; evidence_refs?: ArtifactRef[] }> {
    ...
    if (step.action === "capture_count") { const count = await locate(page, step.locator).count(); (scopes.variables as Record<string, unknown>)[step.save_as] = count; return { summary: { name: step.save_as, count } }; }
    if (step.action === "capture_texts") { const texts = await locate(page, step.locator).allInnerTexts(); (scopes.variables as Record<string, unknown>)[step.save_as] = texts; return { summary: { name: step.save_as, count: texts.length } }; }
...
function locate(page: Page, locator: LocatorRef): Locator {
  const root: Page | Locator = locator.within ? locate(page, locator.within) : page;
  let result: Locator = locator.by === "role" ? root.getByRole(locator.role as any, locator.name === undefined ? {} : { name: locator.name, exact: locator.exact }) : locator.by === "label" ? root.getByLabel(locator.text) : locator.by === "test_id" ? root.getByTestId(locator.value) : locator.by === "text" ? root.getByText(locator.text, { exact: locator.exact }) : locator.by === "id" ? root.locator("#" + locator.value) : root.locator(locator.value);
  if (locator.has_text !== undefined) result = result.filter({ hasText: locator.has_text });
  if (locator.nth !== undefined) result = result.nth(locator.nth);
  return result;
}
...
function jsonPath(value: unknown, expression: string): unknown { return expression.split(".").filter(Boolean).reduce<unknown>((current, part) => { if (part === "[]" || part === "[*]") return Array.isArray(current) ? current : undefined; const arrayField = part.match(/^\[\]([A-Za-z_$][\w$]*)$/); if (arrayField) return Array.isArray(current) ? current.map((item) => jsonPath(item, arrayField[1])) : undefined; const match = part.match(/^(.*)\[(\d+)\]$/); if (match) { const base = match[1] ? current && typeof current === "object" ? (current as Record<string, unknown>)[match[1]] : undefined : current; return Array.isArray(base) ? base[Number(match[2])] : undefined; } const find = part.match(/^@([^=]+)=(.*)$/); if (find) return Array.isArray(current) ? current.find((item) => String(jsonPath(item, find[1])) === find[2]) : undefined; if (Array.isArray(current)) return current.map((item) => jsonPath(item, part)); return current && typeof current === "object" ? (current as Record<string, unknown>)[part] : undefined; }, value); }
function sourceValue(scopes: Record<string, unknown>, source: unknown): unknown {
  if (source && typeof source === "object") return source;
  if (typeof source === "string" && source.startsWith("variables.")) return lookup(scopes.variables, source.slice("variables.".length));
  const response = getSource(scopes, source);
...
function semanticValue(operand: SemanticOperand, scopes: Record<string, unknown>): unknown {
  ...
  const variableSource = operand.source.startsWith("variables.");
  const source = variableSource ? lookup(scopes.variables, operand.source.slice("variables.".length)) : getSource(scopes, operand.source);
  const responseSource = variableSource ? undefined : source as Record<string, unknown>;
  const value = variableSource ? operand.path ? jsonPath(source, operand.path) : source : operand.path?.startsWith("$response.") ? jsonPath(responseSource, operand.path.slice("$response.".length)) : operand.path ? jsonPath(responseSource?.body, operand.path.replace(/^body\./, "")) : responseSource?.body;
...
function classifyFailure(error: unknown, action?: string, phase?: "setup" | "test" | "cleanup" | string, generatedPlan = false): Failure {
  ...
  const failure_signal: FailureSignal = { code, kind: network ? "network" : contract ? "contract" : assertion ? "assertion" : timeout ? "timeout" : code.includes("POLICY") ? "policy" : "unknown", phase: normalizedPhase, ...(action ? { action } : {}), ...(Object.hasOwn(details, "expected") ? { expected: details.expected } : {}), ...(Object.hasOwn(details, "actual") ? { actual: details.actual } : {}) };
  if (network) return { error: message, classification: "INFRA_DEFECT", failure_signal };
  if (assertion && normalizedPhase === "test") return { error: message, classification: "PRODUCT_DEFECT", failure_signal };
