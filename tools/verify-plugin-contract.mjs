...
const pluginSkills = fs.readdirSync(path.join(pluginRoot, "skills"), { withFileTypes: true }).filter((entry) => entry.isDirectory() && fs.existsSync(path.join(pluginRoot, "skills", entry.name, "SKILL.md"))).map((entry) => entry.name);
check("plugin-has-one-unified-skill", pluginSkills.length === 1 && pluginSkills[0] === "web-audit");
