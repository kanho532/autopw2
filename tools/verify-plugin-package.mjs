...
try { const plugin = JSON.parse(fs.readFileSync(path.join(pluginRoot, ".codex-plugin", "plugin.json"), "utf8")); check("plugin-manifest-validates", plugin.name === "autopw" && /^\d+\.\d+\.\d+(?:\+codex\.[A-Za-z0-9._-]+)?$/.test(plugin.version) && plugin.author?.name && plugin.interface?.displayName && plugin.mcpServers === "./.mcp.json"); } catch (error) { check("plugin-manifest-validates", false, String(error)); }
