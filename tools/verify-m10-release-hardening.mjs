...
try {
  ...
  check("m10-default-run-persists-complete-release-metrics", ["static_discovery_wall_ms", "live_discovery_wall_ms", "correlation_cpu_ms", "derivation_cpu_ms", "planner_fill_ms", "compilation_ms", "execution_ms", "report_ms", "total_run_ms"].every((key) => Number.isFinite(metrics[key]) && metrics[key] >= 0), JSON.stringify(metrics));
  ...
  const largeServer = http.createServer((_req, response) => { response.writeHead(200, { "content-type": "application/json" }); response.end(JSON.stringify({ payload: "x".repeat(execution.MAX_API_RESPONSE_BYTES + 128) })); });
  ...
  try {
    ...
    check("m10-response-body-limit-is-enforced", result.results[0]?.status === "FAILED" && String(result.results[0]?.error || "").includes("byte limit"));
