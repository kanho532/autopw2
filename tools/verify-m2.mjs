...
function request(client_request_id) { return { schema_version: "2.1", client_request_id, workspace_id: "ws_demo", project_subpath: ".", profile_path: ".autopw/profile.yaml", base_tier: "fast" }; }

// Pass: a real Chromium run produces terminal artifacts and a pass Gate.
{
  const harness = await newHarness({ stepMs: 2, fixtureVariant: "pass" });
  const accepted = await call(harness.server, "run_audit", request("m2_pass_1"));
  ...
  const resultsArtifact = JSON.parse(fs.readFileSync(path.join(runDir, "artifacts", "results.json"), "utf8"));
  const reportArtifact = fs.readFileSync(path.join(runDir, "artifacts", "report.md"), "utf8");
  check("m2-result-ref-is-resolvable", result.kind === "ok" && result.results_ref.handle.endsWith("_results.json") && resultsArtifact.results_ref.handle === result.results_ref.handle && reportArtifact.includes(result.results_ref.handle), "handle=" + (result.results_ref && result.results_ref.handle));
  const dataRoot = harness.dataRoot;
  await harness.server.stop();
  const restarted = await newHarness({ dataRoot, stepMs: 2, fixtureVariant: "pass" });
...
{
  const harness = await newHarness({ stepMs: 2, fixtureVariant: "fail" });
  const accepted = await call(harness.server, "run_audit", request("m2_fail_1"));
...
{
  const harness = await newHarness({ stepMs: 2, fixtureVariant: "incomplete" });
  const accepted = await call(harness.server, "run_audit", request("m2_incomplete_1"));
